...
In the previous section we showed how easy it is to launch a dashboard template auto populated with device data. However, it is also very straightforward to import custom apps or dashboards and integrate them with OpenEnsemble or any other APIs. 
...
As detailed in [the overview](https://www.fathym.com/openensemble/docs/cloud-apps/overview), Fathym Platform enables you to assemble, deploy and host applications in a highly modular fashion.   
...
One of the application processor types listed above is Proxy and these are used as the integration layer of a project. There are two proxy options: API Proxy and SPA proxy. In the next section we will add an API proxy to connect our application to OpenEnsemble APIs so that it is populated with simulated or real device data. 