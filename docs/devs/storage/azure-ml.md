...
If your using [IoT Ensemble's Best Practice Schema](../../../devs/device-setup/iot-best-practice-schema) then set the **Time column** to `Timestamp`, otherwise choose the appropriate value from your device payload schema or fall back to the system value `EventEnqueuedUtcTime`. 
